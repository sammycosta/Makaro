...
solveByRegion :: GenMatrix Int -> GenMatrix String -> GenMatrix Position -> [[Int]] -> 
                ...
                -> (Bool, GenMatrix Int, GenMatrix Position, [[Int]])
solveByRegion mat matRegions regions regionsPaths wrongPaths possibleNumbers possiblePositions
    | succeeded = (True, newMat, regions, newRegPaths)
    | otherwise = (False, mat, regions, regionsPaths) -- ver o que preciso retornar aq. prob nao vou precisar limpar lá assim! 
    where
        ...
        -- nova regions com a region alterada ? 
